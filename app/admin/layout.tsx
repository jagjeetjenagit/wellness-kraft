...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
      ...
      <div className="container-x flex min-h-[60vh] items-center justify-center py-20">
        <div className="card max-w-lg p-8 text-center">
          ...
          <p className="mt-3 text-sm leading-relaxed text-ink-soft">
            ...
            and bookings. Admins are set in the <code className="rounded bg-cream px-1.5 py-0.5 text-xs">.env</code>{" "}
            file (<code className="rounded bg-cream px-1.5 py-0.5 text-xs">ADMIN_EMAILS</code> /{" "}
            <code className="rounded bg-cream px-1.5 py-0.5 text-xs">ADMIN_PHONES</code>) — README step 2.
          </p>
  ...
  return (
    <div className="container-x py-10">
      {!hasClerk() && (
        <div className="mb-6 rounded-2xl border border-gold/40 bg-gold/10 p-4 text-sm font-semibold text-ink-soft">
      ...
      {!hasDatabase() && (
        <div className="mb-6 rounded-2xl border border-clay/40 bg-clay-light p-4 text-sm font-semibold text-clay">
